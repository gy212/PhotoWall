...
export function SelectionToolbar({ selectedCount, children, className, onClear }: SelectionToolbarProps) {
  return (
    <div className={clsx(
...
export function SelectionAction({ icon, label, className, iconClassName, labelClassName, children, ...props }: SelectionActionProps & { children?: React.ReactNode }) {
  return (
    <button
      className={clsx(
      ...
      {...props}
    >
      <span className={clsx("material-symbols-outlined text-2xl group-hover:scale-110 transition-transform", iconClassName)}>
        {icon}
      </span>
      <span className={clsx("text-[10px] font-medium mt-0.5", labelClassName)}>
        {label}
      </span>
      {children}
    </button>
