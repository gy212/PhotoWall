...
/// 全局 EventSink 存储（用于 worker 线程发送事件）
static EVENT_SINK: RwLock<Option<SharedEventSink>> = RwLock::new(None);

/// 设置全局 EventSink（在应用启动时调用）
pub fn set_event_sink(sink: SharedEventSink) {
...
fn emit_thumbnail_ready(
    ...
    if let Ok(guard) = EVENT_SINK.read() {
        if let Some(ref sink) = *guard {
            ...
            sink.emit_typed("thumbnail-ready", &payload);
        }
    }
}
